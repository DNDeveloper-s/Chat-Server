...
function imageMessageToRoomHandler(nsEndPoint, roomId, data) {
    ...
    if(data.image && data.caption.length > 0) {
        ...
        formData.append('nsEndPoint', nsEndPoint);
        formData.append('time', convertedTime);
        ...
        addMessageToRoom(messageObj, roomId, nsEndPoint);
        loader();

        postEmbeddedMessageToRoom(roomId, formData);
    }
}

async function postEmbeddedMessageToRoom(roomId, formData) {
    const res = await fetch(`${window.location.origin}/message/send?toRoom=true&roomId=${roomId}&embedded=true`, {
        method: "POST",
        body: formData
    });
